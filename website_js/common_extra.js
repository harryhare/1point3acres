    ...
    msg = msg.replace(/(^|>)([^<]+)(?=<|$)/ig, function ($1, $2, $3) {
        for (var j = 0; j < relatedlink.length; j++) {
            if (relatedlink[j] && !relatedid[j]) {
                ...
                var $rtmp = $3;
                if (/^[a-zA-Z]+$/.test(relatedlink[j]['sname'])) {
                    $3 = $3.replace(new RegExp('\\b' + relatedlink[j]['sname'].replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\b'), ra);
                } else {
...
function _updatesecqaa(idhash) {
    if ($('secqaa_' + idhash)) {
        ...
        ajaxget('misc.php?mod=secqaa&action=update&idhash=' + idhash, 'secqaa_' + idhash, null, '', '', function () {
            secST['qaa_' + idhash] = setTimeout(function () {
                $('secqaa_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updatesecqaa(\'' + idhash + '\')">刷新验证问答</span>';
            }, 180000);
        });
    }
}
        ...
        if ($('seccode_' + idhash)) {
            ...
            ajaxget('misc.php?mod=seccode&action=update&idhash=' + idhash, 'seccode_' + idhash, null, '', '', function () {
                secST['code_' + idhash] = setTimeout(function () {
                    $('seccode_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updateseccode(\'' + idhash + '\')">刷新验证码</span>';
                }, 180000);
            });
        }
    } else {
